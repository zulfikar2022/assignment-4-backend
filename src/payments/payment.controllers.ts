...
const makePayment = async (req: Request, res: Response) => {
  try {
    ...
    const { _id: customerId } = jwtDecode<{ _id: string }>(token);
    const customer: TUser = (await User.findById(customerId)) as TUser;
    ...
    shurjopay.config(
      environmentVariables.SP_ENDPOINT!,
      environmentVariables.SP_USERNAME!,
      environmentVariables.SP_PASSWORD!,
      environmentVariables.SP_PREFIX!,
      environmentVariables.SP_RETURN_URL!
    );
    ...
    async (response_data) => {
      ...
      if (response_data[0].bank_status === "Success") {
        try {
          ...
          res.redirect(
            "https://coruscating-bonbon-3879b6.netlify.app/successful-payment"
          );
        }
      } else {
        res.redirect(
          "https://coruscating-bonbon-3879b6.netlify.app/failed-payment"
        );
      }
    },
    (error) => {}
