...
orderRoutes.post("/", auth([Roles.CUSTOMER]), (req, res, next) => {
    try {
...
orderRoutes.delete("/:orderId", auth([Roles.CUSTOMER, Roles.ADMIN]), orderControllers.cancelOrder);
// for fetching a specific order
orderRoutes.get("/:orderId", auth([Roles.CUSTOMER, Roles.ADMIN]), orderControllers.specificOrder);
// for fetching customer specific orders
orderRoutes.get("/customer-order/specific-orders", auth([Roles.CUSTOMER, Roles.ADMIN]), orderControllers.customerSpecificOrders);
